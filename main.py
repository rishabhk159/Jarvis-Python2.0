...
def wishMe():
    ...
    if(hour>=0) and (hour<=12) and ('AM' in t):
        speak(f"Good morning Rishabh, it's {day} and the time is {t}")
    elif(hour>=12)  and (hour<=16) and ('PM' in t):
        speak(f"Good afternoon Rishabh, it's {day} and the time is {t}")
    else:
        speak(f"Good evening Rishabh, it's {day} and the time is {t}")
...
def openApp(command):
    if "calculator" in command:
        ...
        speak("opening paint")
        os.startfile('C:\\Users\\risha\\Desktop\\Paint - Shortcut.lnk')

def closeApp(command):
    if "calculator" in command:
        speak("closing calculator")
        # Use PowerShell to close Calculator (works for UWP)
        os.system('powershell "Get-Process CalculatorApp | Stop-Process -Force"')
        os.system('taskkill /f /im calc.exe')  # Also try classic way
    elif "notepad" in command:
...
def condition():
    ...
    if percentage>=80:
        ...
        speak("Boss we have very low power, please connect to charging otherwise your laptop  should be off...")
